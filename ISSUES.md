...
- [x] Provide normalized child-process arguments where the host defines a
  conventional `argv` mapping. Unix-like targets pass the argument vector
  directly. Windows now serializes each raw `od_spawnvpe()` element with the
  Microsoft C quote and backslash rules before calling `_spawnvpe()`. DOS
  remains compiler-, runtime-, and child-dependent because the operating
  system supplies only a raw command tail. The older `od_spawn()` interface
  retains its preformatted Windows tail for programs which use a nonstandard
