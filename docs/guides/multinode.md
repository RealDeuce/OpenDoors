...
current node through `od_control.od_node`, but the application remains
responsible for choosing node-specific temporary files and protecting shared
application data.
...
Node numbers are useful for log names, work directories, and temporary
protocol files. They are not a substitute for locking shared state. When a BBS
does not report a meaningful node, the application should use its own
