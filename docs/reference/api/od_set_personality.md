...
`od_add_personality()`. The function returns true when a match is selected.
The text-mode implementation reports an empty name as `ERR_PARAMETER` and a
missing name as `ERR_LIMIT`; targets without the personality system return
false with `ERR_UNSUPPORTED`.
