...
sends the ANSI and RIP queries, waits for replies, and enables the
corresponding `od_control.user_ansi` or `user_rip` state when detected. In
local mode it enables ANSI without probing.
