...
`TRUE` if logging is disabled or the log was opened successfully; `FALSE` if
the log file could not be opened.

## Description

The `od_log_open()` function begins OpenDoors log-file processing. It opens the
file named by
...
is `TRUE`, the function performs no file operation and returns `TRUE`. If the
field is `FALSE`, the configured file is opened with the C append mode `"a"`.
Failure to open it returns `FALSE`; the current implementation does not assign
...
`od_log_open()` is intended to begin one logging session. It does not test
