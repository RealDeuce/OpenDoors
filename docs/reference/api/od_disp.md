...
When local echo is disabled, the bytes are transmitted as raw output and are not
inferred into the screen model.

Use [`od_disp_emu()`](od_disp_emu.md) when an ANSI or AVATAR stream must be
interpreted locally. The function returns no value.
