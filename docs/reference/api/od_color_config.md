...
combined attribute used by `od_set_attrib()` and the screen APIs.

Unrecognized words are ignored and the unspecified portions retain their
default, producing grey on black when nothing matches. Applications which
accept configurable colors should use this function rather than maintaining a
second parser.
