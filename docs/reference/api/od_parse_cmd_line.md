...
| `-SOCKET` | decimal number | Sets [`od_control.od_use_socket`](../control/connection.md#od_use_socket) to [`TRUE`](../constants/general.md#true), then stores the result of `atoi()` in [`od_control.od_open_handle`](../control/connection.md#od_open_handle). There is no `-S` alias. |
...
`-ADDRESS` uses `strtol(..., 16)` and then converts the result to [`WORD`](../types.md#word).
