...
Call this function before `od_init()`. The Windows argument is the string
received by `WinMain`; other programs pass `argc` and `argv` from `main`.

Recognized options update initialization settings. Unknown application options
can be handled through `od_control.od_cmd_line_flag_handler` and
`od_control.od_cmd_line_handler`; help text and callbacks are also configurable.
