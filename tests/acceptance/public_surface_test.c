...
int main(void)
{
   ...
   OD_TEST_CHECK(OD_RESERVE_ERROR == -1);
   OD_TEST_CHECK(OD_RESERVE_PENDING == 0);
   OD_TEST_CHECK(OD_RESERVE_ACQUIRED == 1);
   OD_TEST_CHECK(OD_VERSION == 0x630);
   OD_TEST_CHECK(DORINFO1 == 0 && DOOR32SYS == 13 && BBSDEVDRP == 14 &&
      NO_DOOR_FILE == 100);
