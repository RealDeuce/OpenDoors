...
#define UT_CUSTOM_MOCK_ODKrnlRequestTimeValue
...
int WINAPI utm_GetWindowTextA(HWND window,LPSTR text,int size){UT_ASSERT_EQ_PTR((HWND)(UINT_PTR)6,window);UT_ASSERT(size>=3);utm_strcpy(text,"33");return(2);}
int utm_atoi(const char *text){UT_ASSERT(strcmp(text,"33")==0);return(33);}void utm_ODKrnlRequestTimeValue(INT minutes){ut_time_value=minutes;++ut_time_value_calls;}
...
{reset_proc();ut_info.bToolbarOn=TRUE;command(ID_VIEW_TOOL_BAR,0,0);UT_ASSERT(!ut_info.bToolbarOn);UT_ASSERT_EQ_UINT(1,ut_destroy_toolbar_calls);
 ...
 command(ID_USER_INACTIVITYTIMER,0,0);UT_ASSERT_EQ_UINT(1,ut_inactivity_calls);UT_ASSERT(command(ID_TIME_EDIT,0,(LPARAM)(HWND)(UINT_PTR)6));UT_ASSERT_EQ_UINT(0,ut_time_value_calls);UT_ASSERT(command(ID_TIME_EDIT,EN_CHANGE,(LPARAM)(HWND)(UINT_PTR)6));UT_ASSERT_EQ_INT(33,ut_time_value);UT_ASSERT(command(0xffff,0,0));}
static void covers_notifications_scroll_and_resize(void)
{TOOLTIPTEXTA tooltip;static const UINT ids[]={ID_DOOR_EXIT,ID_DOOR_CHATMODE,ID_DOOR_USERKEYBOARDOFF,ID_DOOR_SYSOPNEXT,ID_DOOR_HANGUP,ID_DOOR_LOCKOUT,999};unsigned i;
 ...
 reset_proc();ut_info.hwndTimeUpDown=(HWND)(UINT_PTR)7;call_proc(WM_VSCROLL,MAKEWPARAM(0,44),(LPARAM)(HWND)(UINT_PTR)8);UT_ASSERT_EQ_UINT(0,ut_time_value_calls);call_proc(WM_VSCROLL,MAKEWPARAM(0,44),(LPARAM)ut_info.hwndTimeUpDown);UT_ASSERT_EQ_INT(44,ut_time_value);
