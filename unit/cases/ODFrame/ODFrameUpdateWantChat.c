...
static void reset_chat(void){memset(&ut_info,0,sizeof(ut_info));ut_thread=7;ut_post_calls=ut_lock_depth=ut_status_calls=0;ut_title_calls=ut_flash_calls=ut_kill_calls=ut_timer_calls=0;
 strcpy(od_control.od_prog_name,"Door");od_control.user_wantchat=FALSE;hwndCurrentFrame=(HWND)(UINT_PTR)1;dwFrameThreadID=7;ut_info.hwndStatusBar=(HWND)(UINT_PTR)2;ut_title[0]='\0';}
static void routes_cross_thread_updates(void){reset_chat();hwndCurrentFrame=NULL;utt_ODFrameUpdateWantChat();UT_ASSERT_EQ_UINT(0,ut_post_calls);
 reset_chat();ut_thread=8;utt_ODFrameUpdateWantChat();UT_ASSERT_EQ_UINT(1,ut_post_calls);UT_ASSERT_EQ_UINT(0,ut_lock_depth);}
...
{reset_chat();utt_ODFrameUpdateWantChat();UT_ASSERT_EQ_UINT(0,ut_status_calls);UT_ASSERT_EQ_UINT(0,ut_title_calls);
 reset_chat();ut_info.bStatusBarOn=TRUE;ut_info.bWantsChatIndicator=TRUE;od_control.user_wantchat=TRUE;utt_ODFrameUpdateWantChat();UT_ASSERT_EQ_UINT(1,ut_status_calls);UT_ASSERT_EQ_UINT(0,ut_title_calls);
 reset_chat();ut_info.bWantsChatIndicator=TRUE;od_control.user_wantchat=FALSE;utt_ODFrameUpdateWantChat();UT_ASSERT(strcmp(ut_title,"Door")==0);UT_ASSERT_EQ_UINT(1,ut_flash_calls);UT_ASSERT_EQ_UINT(1,ut_kill_calls);UT_ASSERT(!ut_info.bWantsChatIndicator);
 reset_chat();od_control.user_wantchat=TRUE;utt_ODFrameUpdateWantChat();UT_ASSERT(strcmp(ut_title,"Door - User Wants Chat")==0);UT_ASSERT_EQ_UINT(1,ut_timer_calls);UT_ASSERT(ut_info.bWantsChatIndicator);}
