...
static DWORD ut_thread;static unsigned ut_post_calls,ut_quit_calls,ut_wait_calls,ut_close_calls;
...
static void reset_shutdown(tODThreadHandle *thread){*thread=(HANDLE)(UINT_PTR)1;hwndCurrentFrame=NULL;dwFrameThreadID=7;hFrameStartedEvent=NULL;ut_thread=8;
 ut_post_calls=ut_quit_calls=ut_wait_calls=ut_close_calls=0;}
...
{tODThreadHandle thread;reset_shutdown(&thread);hwndCurrentFrame=(HWND)(UINT_PTR)2;hFrameStartedEvent=(HANDLE)(UINT_PTR)4;
 utt_ODFrameShutdown(&thread);UT_ASSERT_EQ_UINT(1,ut_post_calls);UT_ASSERT_EQ_UINT(0,ut_quit_calls);UT_ASSERT_EQ_UINT(1,ut_wait_calls);UT_ASSERT_EQ_UINT(2,ut_close_calls);UT_ASSERT_NULL(thread);UT_ASSERT_NULL(hFrameStartedEvent);}
