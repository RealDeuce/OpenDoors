...
static unsigned char ut_chars[20];
static const BYTE ut_cursor_rows[] = {3, 4, 5, 6};
...
{
   ++ut_malloc_calls;
   UT_ASSERT_EQ_UINT(sizeof(ut_window), count);
   ...
   void *buffer)
{
   ...
   UT_ASSERT_EQ_UINT(8, right); UT_ASSERT_EQ_UINT(6, bottom);
...
{
   UT_ASSERT(ut_cursor_calls < DIM(ut_cursor_rows));
   UT_ASSERT_EQ_UINT(2, column);
   UT_ASSERT_EQ_UINT(ut_cursor_rows[ut_cursor_calls], row);
   ++ut_cursor_calls;
}
...
{
   ++ut_string_calls;
   if(ut_has_title && ut_string_calls == 1)
      UT_ASSERT(strcmp(string, "HI") == 0);
   else
      UT_ASSERT(strcmp(string, "\272     \272") == 0);
}
...
static void draws_an_untitled_window_and_returns_the_saved_area(void)
{
   ...
   UT_ASSERT_EQ_UINT(1, ut_attribute_calls); UT_ASSERT_EQ_UINT(14, ut_char_calls);
   UT_ASSERT_EQ_UINT(2, ut_string_calls);
   UT_ASSERT_EQ_UINT(214, ut_chars[0]); UT_ASSERT_EQ_UINT(183, ut_chars[6]);
   UT_ASSERT_EQ_UINT(211, ut_chars[7]); UT_ASSERT_EQ_UINT(189, ut_chars[13]);
}
...
static void centers_a_title_with_its_own_attribute(void)
{
   ...
   UT_ASSERT_EQ_UINT(12, ut_char_calls); UT_ASSERT_EQ_UINT(3, ut_string_calls);
